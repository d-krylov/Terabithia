...
void Clear(ClearType clear_type) { glClear(static_cast<uint32_t>(clear_type)); }
...
void DrawArraysInstanced(int32_t first, uint32_t count, uint32_t i_count, PrimitiveKind kind) {
  glDrawArraysInstanced(static_cast<uint16_t>(kind), first, count, i_count);
}
...
void DrawElements(uint32_t count, CoreType i_type, PrimitiveKind kind) {
  glDrawElements(static_cast<uint16_t>(kind), count, static_cast<uint16_t>(i_type), nullptr);
}

void DrawElementsBaseVertex(uint32_t count, uint64_t offset, int32_t base_vertex,
                            CoreType index_type, PrimitiveKind kind) {
...
uint32_t GetClipOrigin() {
  uint32_t ret = 0;
  glGetIntegerv(GL_CLIP_ORIGIN, (int32_t *)&ret);
  return ret;
}
