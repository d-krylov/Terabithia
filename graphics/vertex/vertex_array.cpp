...
void VertexArray::AddVertexBuffer(const VertexBuffer &vbo) {
  ...
  const auto &vertex_format = vbo.GetVertexFormat();
  glVertexArrayVertexBuffer(vertex_array_, binding, vbo, 0, vertex_format.GetStride());
  SetVertexFormat(vertex_format, binding);
}
